...
var config = {
  apiOrigins: {
    production: 'https://tranquil-caverns-61326.herokuapp.com',
...
var apptEvents = __webpack_require__(17);

$(function () {
  setAPIOrigin(location, config);
  authEvents.addHandlers();
  apptEvents.addHandlers();
  $('#sign-out').hide();
...
var onGetAppointments = function onGetAppointments(event) {
  event.preventDefault();
  api.index().then(ui.getAppointmentsSuccess).catch(ui.fail);
};

var onAddAppointment = function onAddAppointment(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.create(data).then(ui.addSuccess).catch(ui.fail);
};

var onUpdateAppointment = function onUpdateAppointment(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  var id = data.appointment.id;
  api.update(id, data).then(ui.updateSuccess).catch(ui.fail);
};

var onDeleteAppointment = function onDeleteAppointment(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.destroy(data.appointment.id).then(ui.deleteSuccess).catch(ui.fail);
};
...
var showAppointmentsTemplate = __webpack_require__(19);
var api = __webpack_require__(7);

var getAppointmentsSuccess = function getAppointmentsSuccess(data) {
  $('#content').text(null);
  $('.clearform').trigger('reset');
  if (data.appointments.length === 0) {
    ...
    $('#list-appts').hide();
    var showAppointmentsHTML = showAppointmentsTemplate({ appointments: data.appointments });
...
var onInlineEdit = function onInlineEdit(event) {
  event.preventDefault();
  $('.inline-edit').hide();
  var id = $(event.target).attr('data-id');
  //
  var desc = $(event.target).parent().siblings()[0];
  desc.contentEditable = true;
  //
  var phoneNumber = $(event.target).parent().siblings()[2];
  phoneNumber.contentEditable = true;
  //
  var apptTime = $(event.target).parent().siblings()[1];
  apptTime.contentEditable = true;
  //
  var reminderBody = $(event.target).parent().siblings()[3];
...
var inlineEdit = function inlineEdit(id, desc, phoneNumber, apptTime, reminderBody) {
  var newDesc = $(desc).html();
  var newPN = $(phoneNumber).html();
  var newTime = $(apptTime).html();
  var newReminder = $(reminderBody).html();
  var data = {
    appointment: {
      name: newDesc,
      time: newTime,
  ...
  if (newDesc !== '' && newPN !== '' && newTime !== '' && newReminder !== '') {
    api.update(id, data).then(updateSuccess).catch(fail);
  }
};

var onInlineDelete = function onInlineDelete(event) {
  var id = $(event.target).attr('data-id');
  api.destroy(id).then(deleteSuccess).catch(fail);
};

var addSuccess = function addSuccess(data) {
  $('#content').text(null);
  $('.clearform').trigger('reset');
  api.index().then(function (data) {
    getAppointmentsSuccess(data);
...
module.exports = {
  ...
  getAppointmentsSuccess: getAppointmentsSuccess,
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "<tr data-id=\""
    ...
    + "</td>\n  <td id=\"appt-reminder\">"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.reminder : stack1), depth0))
    + "</td>\n  <td>\n    <button data-id=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\" class=\"inline-edit btn btn-warning\" value=\"edit\">Edit</button>\n    <button data-id=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\" class=\"inline-delete btn btn-danger\" value=\"delete\">Delete</button>\n    </td>\n</tr>\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;

  return "<table id=\"appointments\">\n  <tr>\n    <th>Appointment Description\n    </th>\n    <th>Appointment Time\n    </th>\n    <th>Phone Number\n    </th>\n    <th>Reminder Message\n    </th>\n  </tr>\n"
    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.appointments : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "</table>\n";
...
// imports
exports.push([module.i, "@import url(https://fonts.googleapis.com/css?family=Berkshire+Swash);", ""]);
exports.push([module.i, "@import url(https://fonts.googleapis.com/css?family=Berkshire+Swash|Open+Sans);", ""]);
